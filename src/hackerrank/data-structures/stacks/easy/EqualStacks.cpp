...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    int s1 = accumulate(h1.begin(), h1.end(), decltype(h1)::value_type(0));
    int s2 = accumulate(h2.begin(), h2.end(), decltype(h2)::value_type(0));
    int s3 = accumulate(h3.begin(), h3.end(), decltype(h3)::value_type(0));
    
    while (s1 != s2 || s1 != s3) {
        int minHeight = min(s1, min(s2, s3));
